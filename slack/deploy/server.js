...
const DEFAULT_CRON_SETTINGS = [
  ...
  { id: "commute",         label: "출근 알림",            schedule: "*/5 8-11 * * 1-5", enabled: true },
  ...
  const fns = {
    ...
    vacationWeekly: async () => {
      try {
        ...
        for (const t of teamsData) { if (!notify(t, "vacationWeekly")) continue; await axios.post(t.webhook, buildVacationMessage(d[0], list.filter(i => t.teams.includes(i.deptName)), "이번주 휴가자", true)); }
      } catch (err) { logger(`[크론 오류] 이번주 휴가 발송 실패: ${err.message}`, true); }
    },
    vacationDaily: async () => {
      try {
        const excluded = vacationExcluded();
        const t = getToday(); const list = (await fetchVacations(t, t, true)).filter(i => !excluded.has(i.usName));
        for (const td of teamsData) { if (!notify(td, "vacationDaily")) continue; await axios.post(td.webhook, buildVacationMessage(t, list.filter(i => td.teams.includes(i.deptName)))); }
    ...
    if (prevVacationSnapshot.map && Object.keys(prevVacationSnapshot.map).length > 0) {
      ...
      if (added.length || removed.length) {
        ...
        for (const t of teamsData) {
          if (t.notify ? t.notify.vacationChange === false : t.enabled === false) continue;
...
async function refreshTeamDropdown(data) {
  ...
  if (!sel) return;
  const cur = sel.value;
  sel.innerHTML = '<option value="">선택</option>' + data.map(t =>
    \`<option value="\${t.deptId||''}">\${t.teams[0]}</option>\`).join('');
  sel.value = cur;
}
