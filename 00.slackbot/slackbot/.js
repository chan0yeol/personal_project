  ...
  const fns = {
    ...
    vacationWeekly: async () => {
      try {
        ...
        const d = thisWeekDates(); const list = (await fetchVacations(d[0], d[4], true)).filter(i => !excluded.has(i.usName));
    ...
    vacationDaily: async () => {
      try {
        const excluded = vacationExcluded();
        const today = getToday(); const list = (await fetchVacations(today, today, true)).filter(i => !excluded.has(i.usName));
        for (const td of dedupeByWebhook("vacationDaily")) { await axios.post(td.webhook, buildVacationMessage(today, list.filter(i => td.teams.includes(i.deptName)))); }
      } catch (err) { logger(`[크론 오류] 오늘 휴가 발송 실패: ${err.message}`, true); }
    },
    roomDaily: async () => {
      try {
        const t = getToday(); const list = await fetchRooms(t, t, true);
        for (const td of teamsData) { if (!notify(td, "roomDaily")) continue; await axios.post(td.webhook, buildRoomMessage(t, list)); }
      ...
      try {
        ...
        for (const deptId of deptIds) {
          ...
          for (const record of commuteList) {
            ...
            // 이미 출근 등록된 경우 스킵
            if (record.workStartTime) continue;
            // 오늘 이미 알림 보낸 경우 스킵
      ...
      try {
        ...
        for (const leader of leaders) {
          ...
          for (const record of rawList) {
            if (!record.usName || !record.workStartTime) continue;
            attendMap[record.usName] = (attendMap[record.usName] || 0) + 1;
          }

          const lines = teamMembers.map(m => {
            const days = attendMap[m.name] ?? 0;
            const ratio = totalWorkdays > 0 ? Math.round((days / totalWorkdays) * 100) : 0;
            const emoji = ratio >= 95 ? "🟢" : ratio >= 80 ? "🟡" : "🔴";
            return `${emoji} *${m.name}*: ${days}일 출근 / ${totalWorkdays}일 근무 (${ratio}%)`;
          }).join("\n");

          const msg = `📊 *${leader.teamName} ${year}년 ${month}월 출근 현황*\n${lines}\n\n_마지막 근무일 기준: ${lastDay}_`;
          await sendSlackDM(leader.slackId, msg);
          logger(`[월말출근리포트] ${leader.teamName} 팀장(${leader.name}) DM 발송`);
        }
...
async function sendSlackDM(userId, text) {
  ...
  try {
    // DM 채널 먼저 열기 (channel_not_found 방지)
    const open = await axios.post("https://slack.com/api/conversations.open",
      { users: userId },
      { headers: { Authorization: `Bearer ${SLACK_BOT_TOKEN}`, "Content-Type": "application/json" } }
    );
    if (!open.data.ok) { logger(`[DM] 채널 오픈 실패 (${userId}): ${open.data.error}`, true); return; }
...
async function refreshTeamDropdown(data) {
  ...
  sel.innerHTML = '<option value="">선택</option>' + data.flatMap(t =>
    t.teams.map(name => \`<option value="\${t.deptId||''}">\${name}</option>\`)).join('');
  sel.value = cur;
}
