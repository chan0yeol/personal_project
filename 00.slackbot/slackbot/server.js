...
const DEFAULT_CRON_SETTINGS = [
  ...
  { id: "commute",         label: "출근 알림",            schedule: "55-59 8-16 * * 1-5", enabled: true },
  ...
  const fns = {
    ...
    commute: async () => {
      try {
        ...
        if (await isHoliday(today)) { logger(`[출근알림] 공휴일 - 스킵 (${today})`); return; }
        ...
        for (const deptId of deptIds) {
          ...
          for (const record of commuteList) {
            ...
            if (record.workStartTime) continue;      // 이미 출근
            if (record.timeUnit === "FULL") continue; // 종일 연차

            // 출근 예정 시간: 반차면 휴가 종료 시간, 아니면 기본 09:00
            const effectiveStime = record.useEtime
              ? `${record.useEtime.slice(0, 2)}:${record.useEtime.slice(2, 4)}`
              : "09:00";
            const [h, m] = effectiveStime.split(":").map(Number);
            const scheduleMin = h * 60 + m;

            if (nowMin < scheduleMin - 5) continue;   // 아직 알림 전
            if (nowMin > scheduleMin + 60) continue;  // sTime + 1시간 초과
            ...
            logger(`[출근알림] DM 발송: ${record.usName} (예정: ${effectiveStime}, 현재: ${nowStr})`);
          }
        }
...
async function isHoliday(dateStr) {
  ...
  try {
    ...
    const url = `https://www.googleapis.com/calendar/v3/calendars/${calId}/events?key=${HOLIDAY_API_KEY}&timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}&singleEvents=true`;
    const res = await axios.get(url);
    const result = (res.data?.items?.length ?? 0) > 0;
    holidayCache[dateStr] = result;
    if (result) logger(`[공휴일] ${dateStr} - ${res.data.items[0].summary}`);
    return result;
  } catch (err) {
    logger(`[공휴일 조회 오류] ${err.message}`, true);
    return false;
  }
}
...
app.patch("/api/members/:index/toggle/:type", (req, res) => {
  const idx = parseInt(req.params.index);
  if (isNaN(idx) || idx < 0 || idx >= membersData.length) return res.status(404).json({ error: "없는 인덱스" });
  const { type } = req.params;
  ...
  if (type === "leader") {
    ...
    // commute, vacation 모두 기본값 true(켜짐)
