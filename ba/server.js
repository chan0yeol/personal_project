...
const LOG_PATH = process.env.LOG_PATH || path.join(__dirname, "server.log");
// ==========================
...
async function syncAndCheckChanges(isManual = false) {
  ...
  try {
    ...
    if (prevVacationSnapshot.map && Object.keys(prevVacationSnapshot.map).length > 0) {
      ...
      const removed = Object.values(prevVacationSnapshot.map).filter(i => !vCurrMap[vKey(i)] && (i.useEdate ?? i.useSdate ?? "") >= today);
...
function buildFooter(type = "vacation") {
  ...
  const lastCheck = snapshot.updatedAt ? new Date(snapshot.updatedAt).toLocaleString("ko-KR", { hour: "2-digit", minute: "2-digit", hour12: true, timeZone: "Asia/Seoul" }) : "기록 없음";
  ...
  try {
    ...
    if (command === "/휴가") {
      ...
      if (matchedTeam) {
        ...
        else message = buildVacationMessage(sdate, list.filter(i => TEAM_WEBHOOKS.some(t => t.teams.includes(i.deptName))), `${titleDate} 전체 휴가자`, isRange, TEAM_WEBHOOKS);
      }
    } else if (command === "/회의실") {
...
cron.schedule("0 9 * * 1", async () => { try { const d = (()=>{ const d = new Date(); const mon = new Date(d); mon.setDate(d.getDate() - d.getDay() + 1); return Array.from({length:5},(_,i)=>{ const nd = new Date(mon); nd.setDate(mon.getDate()+i); return formatDate(nd); }); })(); const list = await fetchVacations(d[0], d[4], true); for (const { teams, webhook } of TEAM_WEBHOOKS) await axios.post(webhook, buildVacationMessage(d[0], list.filter(i => teams.includes(i.deptName)), "이번주 휴가자", true)); } catch (err) { logger(`[크론 오류] 이번주 휴가 발송 실패: ${err.message}`, true); } }, { timezone: "Asia/Seoul" });
cron.schedule("0 9 * * 2-5", async () => { try { const t = getToday(); const list = await fetchVacations(t, t, true); for (const { teams, webhook } of TEAM_WEBHOOKS) await axios.post(webhook, buildVacationMessage(t, list.filter(i => teams.includes(i.deptName)))); } catch (err) { logger(`[크론 오류] 오늘 휴가 발송 실패: ${err.message}`, true); } }, { timezone: "Asia/Seoul" });
cron.schedule("0 9 * * 1-5", async () => { try { const t = getToday(); const list = await fetchRooms(t, t, true); for (const { webhook } of TEAM_WEBHOOKS) await axios.post(webhook, buildRoomMessage(t, list)); } catch (err) { logger(`[크론 오류] 오늘 회의실 발송 실패: ${err.message}`, true); } }, { timezone: "Asia/Seoul" });
cron.schedule("0 0 * * 1", () => { try { fs.writeFileSync(LOG_PATH, "", "utf8"); logger("로그 파일 초기화"); } catch (e) {} }, { timezone: "Asia/Seoul" });
cron.schedule("*/30 * * * *", async () => { try { await axios.get(SESSION_KEEP_URL, { headers: { Cookie: avsCookie }, validateStatus: s => true }); } catch { await loginAvs().catch(e => {}); } try { await axios.get(GW_SESSION_URL, { headers: { Cookie: gwCookie }, validateStatus: s => true }); } catch { await loginGw().catch(e => {}); } });

async function runTest() {
  logger(`===== 테스트 모드 시작 =====`);
  await loginAll();

  if (!TEAM_WEBHOOKS.length) { logger("TEAM_WEBHOOKS 설정 없음", true); process.exit(1); }
  ...
  for (const target of TEAM_WEBHOOKS) {
