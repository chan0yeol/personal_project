...
app.get("/api/today-vacation", async (req, res) => {
  ...
  try {
    ...
    // 응답 구조 확인용 로그 (디버깅 후 제거 가능)
    console.log("response.data 구조:", JSON.stringify(response.data, null, 2).slice(0, 500));

    // 응답 구조에 따라 배열 추출 (다양한 경우 대응)
    let rawList = [];
    if (Array.isArray(response.data)) {
      ...
      rawList = response.data.result;
    } else {
      // 배열을 못 찾은 경우 전체 구조를 그대로 반환해서 확인
      return res.status(200).json({
        debug: true,
        message: "배열을 찾지 못했습니다. 아래 구조를 확인해주세요.",
